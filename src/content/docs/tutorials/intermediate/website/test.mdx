...
Testing a static website is about as low-stress as software testing gets — no servers
to restart, no databases to seed, no deployment pipeline to untangle. Open a browser,
click around, resize the window, and see what happens. The goal is to catch anything
...
<ol>
  <li>
    ...
    **From the GUI:** Double-click `index.html` in Finder or File Explorer. Your
    default browser will open it.

    The URL in your address bar will look something like
    `file:///Users/yourname/projects/my-profile-site/index.html`. That `file://`
    prefix means the browser is reading directly from your disk rather than a web
    server — which is perfectly fine for everything built in this tutorial.
  </li>
  <li>
    ### (Optional) Serve it over HTTP instead

    If you added any `fetch()` calls or ES module imports beyond what is in this
    tutorial, the browser's security policy will block them over `file://`.
...
<Aside type="note">
  ...
  Chrome on an iPhone is actually Safari underneath. It is a long story.[^1]
</Aside>
